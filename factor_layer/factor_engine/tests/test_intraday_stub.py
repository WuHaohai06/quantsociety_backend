"""INTRADAY_STUB_OPS：编译与执行占位（与 test_operators_stub 同模式）。"""
...
@pytest.mark.parametrize("op", sorted(INTRADAY_STUB_OPS))
def test_intraday_stub_compile(op):
    fn = getattr(ops, op)
    expr = fn(col("x"))
...
@pytest.mark.parametrize("op", sorted(INTRADAY_STUB_OPS))
def test_intraday_stub_execute_raises(op):
    fn = getattr(ops, op)
    expr = fn(col("x"))
